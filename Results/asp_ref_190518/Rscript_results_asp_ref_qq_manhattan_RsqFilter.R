...
rsq_filter <- readRDS("files/GxEScanR_GxE_asp_ref_Rsq_MAF_IDsOnly.rds")
...
# QQ Plot
png("figures/GxEScanR_asp_ref_chiSqG_qq.png", height = 720, width = 1280)
qqman::qq(results_G$P, 
          xlab = "Expected -log10(p)", 
          ylab = "Observed -log10(p)",
          main = "G Main Effects Results (N = 72,820)\noutcome ~ dosage+age_ref_imp+sex+study_gxe+PC1-3+asp_ref",
...
write.table(results_G_EasyStrata, file = "/media/work/tmp/EasyStrata_MarginalG_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("figures/EasyStrata_MarginalG_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.ecf")
...
png("figures/GxEScanR_asp_ref_chiSqGxE_qq.png", height = 720, width = 1280)
qqman::qq(results_GxE$P, 
          xlab = "Expected -log10(p)", 
          ylab = "Observed -log10(p)",
          main = "GxNSAIDs Results (N = 72,820)\noutcome ~ G*asp_ref+age_ref_imp+sex+study_gxe+PC1-3",
          #sub = bquote(lambda ~ '=' ~ .(signif(lambda, 4)) ~~ lambda[1000] ~ '=' ~.(signif(lambda1000, 4))),
          cex.main = 1.6, 
          cex.axis = 1.3, 
          cex.lab = 1.3,
...
write.table(results_GxE_EasyStrata, file = "/media/work/tmp/EasyStrata_GxE_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("figures/EasyStrata_GxE_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.ecf")
...
results_2df <- gxe %>%
  mutate(ID = paste(SNP, Reference, Alternate, sep = ":"),
         P = pchisq(chiSq2df, df = 2, lower.tail = F)) %>%
  dplyr::select(SNP, ID, Chromosome, Location, Subjects, Cases, Reference, Alternate, betaG, P) %>% 
  filter(ID %in% rsq_filter$ID)
...
qqman::qq(results_2df$P, 
          xlab = "Expected -log10(p)", 
          ylab = "Observed -log10(p)",
          main = "G/GxNSAIDs 2DF Results\noutcome ~ G+G*asp_ref+age_ref_imp+sex+study_gxe+PC1-3",
          #sub = bquote(lambda ~ '=' ~ .(signif(lambda, 4)) ~~ lambda[1000] ~ '=' ~.(signif(lambda1000, 4))),
          cex.main = 1.6, 
          cex.axis = 1.3, 
          cex.lab = 1.3,
...
write.table(results_2df_EasyStrata, file = "/media/work/tmp/EasyStrata_2df_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("figures/EasyStrata_2df_asp_ref_sex_age_pc_studygxe_N72820_RsqFilter.ecf")
...
write.table(results_G_2df_miami, file = "/media/work/tmp/EasyStrata_Miami_G_2df_asp_ref_sex_age_pc_studygxe_N72820.txt", quote = F, row.names = F, sep = '\t')


EasyStrata("figures/EasyStrata_Miami_G_2df_asp_ref_sex_age_pc_studygxe_N72820.ecf")
...
png("./figures/GxEScanR_asp_ref_sex_age_pc_studygxe_N72820_KooperbergDG_Weighted.png", width = 1280, height = 720)
...
png("./figures/GxEScanR_asp_ref_sex_age_pc_studygxe_N72820_MurcrayGE_Weighted.png", width = 1280, height = 720)
color <- rep(c("blue","olivedrab4"),100)
plot(glist[[1]][,x], glist[[1]][,y], 
     col = "blue", 
     xlab="Bin # for step1 p-values", 
