...
rm(list = ls())
setwd("~/Dropbox/code/FIGI_Results/sex_age_pc10_studyname/")
source("~/Dropbox/code/FIGI_Results/sex_age_pc10_studyname/functions.R")

# original results
results <- do.call(rbind, lapply(list.files(full.names = T, pattern = "results_sex_age_pcs_studyname_N101171"), fread, stringsAsFactors = F))
names(results)
...
write.table(results_G_EasyStrata, file = "~/bin/EasyStrata/results_G_sex_age_pcs_studyname_N101171.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("~/Dropbox/code/FIGI_Results/sex_age_pc10_studyname/results_sex_EasyStrata_G.ecf")
...
png("GxEScanR_sex_chiSqGxE_qq.png", height = 600, width = 800)
qqman::qq(results_GxE$P, 
          xlab = "Expected chi-squared value", 
          ylab = "Observed chi-squared value",
          main = "GxE Results\noutcome ~ Gdosage*sex+age_ref_imp+studyname+PC1-10",
          sub = bquote(lambda ~ '=' ~ .(signif(lambda, 4)) ~~ lambda[1000] ~ '=' ~.(signif(lambda1000, 4)))) # ~~ adds space lol
dev.off()

results_man <- filter(results_GxE, P < 0.05)
png("GxEScanR_sex_chiSqGxE_manhattan.png", width = 800, height = 600)
qqman::manhattan(results_man, main = "GxE Results\noutcome ~ Gdosage*sex+age_ref_imp+sex+studyname+PC1-10")
dev.off()
...
write.table(results_GxE_EasyStrata, file = "~/bin/EasyStrata/results_GxE_sex_age_pcs_studyname_N101171.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("~/Dropbox/code/FIGI_Results/sex_age_pc10_studyname/results_sex_EasyStrata_GxE.ecf")
...
png("~/Dropbox/working/GxEScanR_asp_ref_chiSq2df_qq.png", height = 600, width = 800)
qqman::qq(results_GGxE$P, 
          xlab = "Expected chi-squared value (2df)", 
          ylab = "Observed chi-squared value (2df)",
          main = "G,GxE Joint Model Results\noutcome ~ age_ref_imp+sex+studyname+PC1-10+asp_ref",
          sub = bquote(lambda ~ '=' ~ .(signif(lambda, 4)) ~~ lambda[1000] ~ '=' ~.(signif(lambda1000, 4)))) # ~~ adds space lol
dev.off()

results_man <- filter(results_GGxE, P < 0.05)
png("~/Dropbox/working/GxEScanR_asp_ref_chiSq2df_manhattan.png", width = 800, height = 600)
qqman::manhattan(results_man, main = "G,GxE Joint Model Results\noutcome ~ age_ref_imp+sex+studyname+PC1-10+asp_ref")
dev.off()
