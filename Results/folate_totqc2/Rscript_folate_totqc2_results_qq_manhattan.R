...
gxe_all <- do.call(rbind, lapply(list.files(path = "~/data/Results/folate_totqc2/", full.names = T, pattern = "FIGI_GxESet_folate_totqc2_sex_age_pc3_energytot_studygxe_54084_chr"), fread, stringsAsFactors = F)) %>% 
...
for(chr in 1:22) {
  out <- calculate_pval(gxe, 'chiSqGxE', df = 1) %>%
    ...
    dplyr::select(SNP, P)
  write.table(out, file = paste0("/media/work/tmp/Plink_folate_totqc2_ldclump_chiSqGxE_chr", chr, ".txt"), quote = F, row.names = F, sep = '\t')
}

# LD Clump Results
tmp <- do.call(rbind, lapply(list.files("~/data/Results/folate_totqc2/clump/", full.names = T, pattern = "*.clumped"), fread, stringsAsFactors = F))
gxe_chiSqGxE_ld <- gxe %>% 
