...
# I think remove all 'iffy' UKB markers is prudent
# not worth keeping them for sake of annotation if they're going to potentially lead to misleading GxE results
x <- unique(fh_annotations$SNP)
tmp1 <- filter(ukb_allele_check,  SNP %in% x, P < 5e-8)
tmp2 <- filter(ukb_allele_check, !SNP %in% x, P < 5e-8)
ukb_filter <- rbind(tmp1, tmp2)
any(duplicated(ukb_filter$SNP))
rm(tmp1); rm(tmp2); rm(x); rm(ukb_allele_check)
...
# }
# 
# EasyStrata("results_asp_ref_EasyStrata_G_LDAnnot_ukb_filter.ecf") # have to change file manually...
# 
...
# Manhattan Plot
results_GxE_EasyStrata_ukb_filter <- results_GxE %>% 
  mutate(annot = ifelse(SNP %in% fh_annotations$SNP, 1, 0)) %>% 
  filter(!(P > 0.05 & annot == 0)) %>% 
  dplyr::rename(CHR = Chromosome,
                BP = Location, 
...
write.table(results_GxE_EasyStrata_ukb_filter, file = "~/data/Annotations/EasyStrata_GxE_asp_ref_sex_age_pcs_studygxe_N72145_ukb_filter.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("results_asp_ref_EasyStrata_GxE_LDAnnot_ukb_filter.ecf")
...
results_case <- results %>%
  mutate(P = pchisq(ChiSqCase, df = 1, lower.tail = F))

lambda <- getlambda(results_case$P)

CairoPNG(height = 720, width = 1280, file = "figures/GxEScanR_asp_ref_chiSqCase_qq.png")
qqman::qq(results_case$P, 
          xlab = "Expected -log10(p)", 
          ylab = "Observed -log10(p)",
          main = "GxNSAIDs Case-Only Analyses QQ Plot",
          cex.main = 1.8, 
          cex.axis = 1.5, 
          cex.lab = 1.5,
          cex.sub = 1.5,
          col = 'blue4')
par(adj = 1)
title(sub = bquote(lambda ~ '=' ~ .(signif(lambda, 4))), cex.sub = 1.5)
dev.off()


# Manhattan Plot
results_case_easystrata <- results_case %>% 
  mutate(annot = ifelse(SNP %in% fh_annotations$SNP, 1, 0)) %>% 
  filter(!(P > 0.05 & annot == 0)) %>% 
  dplyr::rename(CHR = Chromosome,
                BP = Location, 
...
write.table(results_case_easystrata, file = "~/data/Annotations/EasyStrata_GxE_CaseOnly_asp_ref_sex_age_pcs_studygxe_N72145_ukb_filter.txt", quote = F, row.names = F, sep = '\t')

EasyStrata("results_asp_ref_EasyStrata_GxE_CaseOnly_LDAnnot_ukb_filter.ecf")
...
huh2 <- edgxe_subset %>% 
  filter(pval_GxE < bonferroni_p_liberal)

huhf <- inner_join(huh, huh2, by = "Location")
