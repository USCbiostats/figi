...
# checks
xtabs(~ cov_gxe$aspirin, addNA = T)
xtabs(~ cov_gxe$sex, addNA = T)
xtabs(~ cov_gxe$study_gxe, addNA = T)
xtabs(~ cov_gxe$outc + cov_gxe$aspirin, addNA = T)


# remove missing aspirin, recode covariates
cov <- cov_gxe %>% 
  filter(aspirin != "") %>% 
...
# save files
saveRDS(cov,         file = '~/data/GxEScanR_PhenotypeFiles/FIGI_GxESet_aspirin_sex_age_pc3_studygxe_72269_GLM.rds', version = 2)
saveRDS(cov_gxescan, file = "~/data/GxEScanR_PhenotypeFiles/FIGI_GxESet_aspirin_sex_age_pc3_studygxe_72269.rds",     version = 2)
