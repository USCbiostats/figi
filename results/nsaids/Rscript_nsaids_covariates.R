...
# checks
xtabs(~ cov_gxe$nsaids, addNA = T)
xtabs(~ cov_gxe$sex, addNA = T)
xtabs(~ cov_gxe$study_gxe, addNA = T)
xtabs(~ cov_gxe$outc + cov_gxe$nsaids, addNA = T)


# remove missing nsaids, recode covariates
cov <- cov_gxe %>% 
  filter(nsaids != "") %>% 
  ...
  dplyr::select(vcfid, outcome, age_ref_imp, sex, study_gxe, paste0(rep("PC", 3), seq(1,3)), nsaids) %>% 
...
cov_gxescan <- dplyr::select(cov_gxescan, -Kentucky, -study_gxe, -nsaids, nsaids)

# save files
saveRDS(cov,         file = '~/data/GxEScanR_PhenotypeFiles/FIGI_GxESet_nsaids_sex_age_pc3_studygxe_70335_GLM.rds', version = 2)
saveRDS(cov_gxescan, file = "~/data/GxEScanR_PhenotypeFiles/FIGI_GxESet_nsaids_sex_age_pc3_studygxe_70335.rds",     version = 2)
