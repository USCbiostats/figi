...
filename = 'FIGI_v2.3_gxeset_alcoholc_heavy_vs_moderate_basic_covars_gxescan'
covars = c('age_ref_imp', 'sex', 'study_gxe', 'energytot_imp', 'pc1', 'pc2', 'pc3')
...
create_manhattanplot(gxe, exposure, covars, 'chiSq2df', 1, annotation_file = annotation, filename_suffix = "_GECCO_Presentation")
create_manhattanplot(gxe, exposure, covars, 'chiSq3df', 1, annotation_file = annotation, filename_suffix = "_GECCO_Presentation")
...
create_manhattanplot(gxe_nogwas, exposure, covars, 'chiSq2df', 1, annotation_file = annotation, filename_suffix = "_GECCO_Presentation_NOGWAS")
create_manhattanplot(gxe_nogwas, exposure, covars, 'chiSq3df', 1, annotation_file = annotation, filename_suffix = "_GECCO_Presentation_NOGWAS")



# generate locuszoom plots (for the 'novel' 2df peaks)
loc <- filter(gxe, Chromosome == 18) %>% arrange(desc(chiSqG))
head(loc)
loc2 <- filter(gxe_nogwas, Chromosome == 18) %>% arrange(desc(chiSqG))
head(loc2)


loc <- filter(gxe, Chromosome == 20) %>% arrange(desc(chiSqG))
head(loc)
loc2 <- filter(gxe_nogwas, Chromosome == 20) %>% arrange(desc(chiSqG))
head(loc2)
...
loc <- filter(gxe, Chromosome == 1) %>% arrange(desc(chiSqG))
head(loc)
loc2 <- filter(gxe_nogwas, Chromosome == 1) %>% arrange(desc(chiSqG))
head(loc2)
