...
results <- cov_df %>% 
  ...
  purrr::map(~glm(cov_df$outcome ~ .x + cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3+cov_df$PC4+cov_df$PC5+cov_df$PC6+cov_df$PC7+cov_df$PC8+cov_df$PC9+cov_df$PC10) %>% tidy, family = 'binomial') 

saveRDS(results, file = "~/Dropbox/FIGI/Code/Misc/Josh_20190916/results_wald_pc10_124702.rds", version = 2)
...
results <- cov_df %>% 
  select(names(j2)[grepl("X", names(j2))]) %>% 
  purrr::map(~lrtest(glm(cov_df$outcome ~ .x + cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3+cov_df$PC4+cov_df$PC5+cov_df$PC6+cov_df$PC7+cov_df$PC8+cov_df$PC9+cov_df$PC10),
                     glm(cov_df$outcome ~      cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3+cov_df$PC4+cov_df$PC5+cov_df$PC6+cov_df$PC7+cov_df$PC8+cov_df$PC9+cov_df$PC10)), family = 'binomial') 
...
write.csv(results_final, file = "~/Dropbox/FIGI/Code/Misc/Josh_20190916/results_clean_lrtest_pc10_124702.csv", quote = F, row.names = F)
