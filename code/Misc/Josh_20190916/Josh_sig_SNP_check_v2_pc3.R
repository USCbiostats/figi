...
results <- cov_df %>% 
  ...
  purrr::map(~glm(cov_df$outcome ~ .x + cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3) %>% tidy, family = 'binomial') 

saveRDS(results, file = "~/Dropbox/FIGI/Code/Misc/Josh_20190916/results_wald_pc3_124702.rds", version = 2)
...
results <- cov_df %>% 
  ...
  purrr::map(~lrtest(glm(cov_df$outcome ~ .x + cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3),
                     glm(cov_df$outcome ~      cov_df$age_ref_imp+cov_df$sex+cov_df$study_gxe+cov_df$PC1+cov_df$PC2+cov_df$PC3)), family = 'binomial') 
