...
cov <- figi %>%
  ...
  mutate(outcome = ifelse(outc == "Case", 1, 0),
         ...
         folate_totqc2 = ifelse(folate_totqc2 == "1", 1,
                         ifelse(folate_totqc2 == "2", 2, 
                         ifelse(folate_totqc2 == "3", 3, 
                         ifelse(folate_totqc2 == "4", 4, NA))))) %>% 
  dplyr::select(vcfid, outcome, age_ref_imp, sex, study_gxe, paste0(rep("PC", 3), seq(1,3)), folate_totqc2, contains('alcohol')) %>% 
