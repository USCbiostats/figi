...
#'
#' @param chiSq Vector of chiSq statistics (typically from GWIS)
...
#' write_plot_title
#'
#' Function to create figure titles based on statistic plotted
#'
...
format_data_twostep_data <- function(dat, stats_step1, sizeBin0, alpha) {
...
# file name =D
write_twostep_weightedHT_plot_title <- function(statistic) {
...
#' create_twostep_weightedHT_plot
#'
...
#'
#' @examples
create_twostep_weightedHT_plot <- function(dat, exposure, covars, sizeBin0, alpha, binsToPlot, statistic) {
  ...
  write_twostep_weightedHT_plot_title("chiSqG")
  ...
  # title(main = write_twostep_weightedHT_plot_title(statistic), sub = "iBin Size = 5, alpha = 0.05", cex.main = 1.5, cex.sub = 1.2)
  #
  # dev.off()
